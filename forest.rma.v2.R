forest.rma.v2 <- function (x, annotate = TRUE, addfit = TRUE, addcred = FALSE, 
          ...
          cex, cex.lab, cex.axis, annosym, col.polygon, ...) 
{
  if (!inherits(x, "rma")) 
  ...
  #tmp <- cbind(ci.lb, ci.ub)
  ci.lb <- tmp[, 1]
  ci.ub <- tmp[, 2]
  
  
  tmp <- metafor:::.psort(pred.ci.lb, pred.ci.ub)
  #tmp <- cbind(pred.ci.lb, pred.ci.ub)
  pred.ci.lb <- tmp[, 1]
  pred.ci.ub <- tmp[, 2]
  ...
  if (addfit && x$int.only) {
    ...
    tmp <- metafor:::.psort(beta.ci.lb, beta.ci.ub)
    #tmp <- cbind(beta.ci.lb, beta.ci.ub)
    beta.ci.lb <- tmp[, 1]
    beta.ci.ub <- tmp[, 2]
    
    
    tmp <- metafor:::.psort(beta.cr.lb, beta.cr.ub)
    #tmp <- cbind(beta.cr.lb, beta.cr.ub)
    beta.cr.lb <- tmp[, 1]
    beta.cr.ub <- tmp[, 2]
  ...
  axis(side = 1, at = at, labels = at.lab, cex.axis = cex.axis, 
       ...)
  if (missing(xlab)) 
    xlab <- metafor:::.setlab(measure, transf.char, atransf.char, gentype = 1)
      ...
      polygon(x = c(alim[2], alim[2] - (1.4/100) * cex * 
                      ...
                      (xlim[2] - xlim[1]), alim[2]), y = c(rows[i], 
                                                           rows[i] + (height/150) * cex * efac[2], rows[i] - 
                                                             (height/150) * cex * efac[2], rows[i]), col = col.polygon[i], 
              ...)
      next
    }
    if (ci.ub[i] <= alim[1]) {
